...
# -----------------------------------------------------------------------------
# ----- source driven two region kinetics solver class 
# -----------------------------------------------------------------------------
...
class srcpke2region:
    ...
    def __init__(self, inputs):
        """function initalizes source driven two region kinetics solver class"""
    ...
    def __findinitalpopulation(self, ni):
        """function calculates inital reactor power using newton-raphson
        method"""
        
        delayed = np.sum(self.inputs.beta / (self.inputs.promptLc) ) * ni
        
        alpha1 = (self.inputs.rhoi-self.inputs.betaTot-self.inputs.f*\
                  (1-self.inputs.betaTot))/(self.inputs.promptLc*(1-self.inputs.f))
        alpha2 = (self.inputs.frc*(1-self.inputs.rhoi))/(self.inputs.promptLr*\
                  (1-self.inputs.f))
        promptcore = alpha1*ni+alpha2*self.inputs.fcr*\
            (self.inputs.promptLr/self.inputs.promptLc)*ni
        
        xi1 = (self.inputs.fcr/self.inputs.promptLc)*((1-self.inputs.rhoi)/\
                (1-self.inputs.f))
        xi2 = (1/self.inputs.promptLr)*((1-self.inputs.rhoi)/(1-self.inputs.f))
        promptreflector = xi1*ni - xi2*self.inputs.fcr*\
            (self.inputs.promptLr/self.inputs.promptLc)*ni
        
        balance = promptcore + promptreflector + delayed + self.inputs.srcnt
        
        return balance
    
    
    def __solveinitialconditions(self, rtol):
        """function finds initial conditions for  source driven two region
        kinetics"""
        ...
        #initialize core neutron population
        x0[0] = self.inputs.n0
        #initalize reflector neutron population
        x0[1] = self.inputs.fcr*(self.inputs.promptLr/self.inputs.promptLc)*self.inputs.n0
        #initialize precusor conc.
        x0[2:] = self.inputs.n0*self.inputs.beta/(self.inputs.lamda*self.inputs.promptLc)
    ...
    def __constructmatrix(self, t):
        ...
        #compute total reactivity of the system
        rho = self.inputs.rhoext.evaluate(t) + self.inputs.rhoi
    ...
    def __postprocess(self, solution):
        ...
        self.outputs = \
            pointkineticsOutputsContainer(timepoints=self.inputs.timepoints,\
                ntc=ntc, ntr=ntr, dnt=dnt, power=power, rho=rho, fluxc=fluxc,
                fluxr=fluxr, typ="srcpke2region")
        
    
    def solve(self, rtol=1e-10):
        """function solver source driven two-region kinetics problem"""
